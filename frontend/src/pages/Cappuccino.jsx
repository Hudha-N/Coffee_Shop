...
const Cappuccino = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/coffees?category_id=3")
  ...
  const handleAddToCart = (coffee) => {
    ...
    if (!existing) {
      cart.push({ id: productId, product_id: productId, name: coffee.name || coffee.coffee_name, image: coffee.image || coffee.image_url || '', price: coffee.price || 0, quantity: 1 });
    }
    localStorage.setItem("cart", JSON.stringify(cart));
    fetch('http://localhost:5000/api/cart', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: userId, items: cart }) }).catch(() => {});
    ...
    <div>
      ...
      <div className={styles.headerSection}>
        <nav className={styles.breadcrumbNav}>
          ...
          <span className={styles.breadcrumbCurrent}>Cappuccino Coffees</span>
        </nav>

        <h2 className={styles.pageTitle}>Cappuccino Coffees</h2>
        ...
        <div className={styles.searchWrapper}>
          <MeltyInput
            ...
            placeholder="Search cappuccino types…"
            width="300px"
            height="36px"
            radius={18}
          />
      ...
      <div className={styles.grid}>
       {filteredCoffees.length > 0 ? (
            ...
            <img src={coffee.image} alt={coffee.coffee_name} className={styles.image} />
            ...
            {selectedCoffee === coffee.id && (
              ...
              >
                Add to Cart
                  ...
                  <p className={styles.noResults}>No cappuccino coffees found</p>
                )}
      </div>
      </div>
    </div>
